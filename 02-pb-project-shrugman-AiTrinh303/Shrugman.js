...
=======
  constructor(options) {
    ...
    this.stats = []; // array containing the game stats
    this.options = options; // Object containing the categories and words
    this.category = this.setCategory(); // Category of the current game
    this.currentWord = this.getSecretWord(this.category); // Current word to guess
  }

  //1. Set the category of the game
  setCategory() {
    let categoryChoice = promptSync(`Choose a category by entering the number:
Movies [0]
Books [1]
Famous Quotes [2]`);
    ...
    // }
    this.category = Object.keys(this.options)[categoryChoice]; // Set the category
    return this.category; // Return true if the category exists
  }
  ...
  getSecretWord(category) {
    ...
    if (
      this.playedWords[this.category].length ===
...
>>>>>>> 7fc51589b70be6a606e3a43f274e22f5826f5f79
  ...
  renderWord() {
    return this.currentWord
      .split("")
      .map((char) =>
        this.knownLettersList.includes(char.toLowerCase()) || char === " "
          ? char
          : " _"
      )
      .join("");
