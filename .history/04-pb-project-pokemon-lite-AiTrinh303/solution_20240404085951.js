...
class Pokemon {
    ...
    attack(skillName, opponent) {
        ...
        if (!this.isAlive()) {
            console.log(`${this.name} cannot attack. It is already defeated after ${this.counter} attacks!`);
            return;
        }

        if (!opponent.isAlive()) {
            console.log(`${opponent.name} is already defeated after ${this.counter} attac !`);
            return;
        }
