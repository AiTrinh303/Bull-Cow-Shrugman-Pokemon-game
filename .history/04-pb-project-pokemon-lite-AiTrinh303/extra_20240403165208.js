...
// Count the number of wins for all Pokemon in the newPokemons array
function countAllWins() {
  for (let i = 0; i < newPokemons.length-1; i++) {
    const currentPokemon = newPokemons[i]; // Get the current Pokemon
    for (let j = i + 1; j < newPokemons.length; j++) {    const opponentPokemon = newPokemons[j]; // Get the opponent Pokemon
