...
function levelSelector() {
  ...
  // Function to give maxAttempts based on the level name
function getMaxAttempts(level) {
    switch (level) {
        case "easy":
        case "es":
            return Infinity; // No limit on attempts
        case "easy+":
        case "es+":
            return 10; // 10 attempts limit
        case "medium":
        case "m":
            return Infinity; // No limit on attempts
        case "medium+":
        case "m+":
            return 15; // 15 attempts limit
        case "hard":
        case "h":
            return Infinity; // No limit on attempts
        case "hard+":
        case "h+":
            return 20; // 20 attempts limit
        case "extreme":
        case "ex":
            return Infinity; // No limit on attempts
        case "extreme+":
        case "ex+":
            return 30; // 30 attempts limit
...
function playTheGame(level) {
    ...
    while (attempts < getMaxAttempts(level) && guess !== secretNumber) {
      attempts++;
    ...
    console.log(`\nGame over! You couldn't guess the number in ${maxAttempts} attempts. The secret number is: ${secretNumber}`);
  }
