...
// function evaluateGuess(secret, guess) {
//     const bulls = secret.split('').filter((digit, index) => guess[index] === digit).length;
    
//     const secretDigitsCount = secret.split('').reduce((obj, digit) => {
//       obj[digit] = obj[digit] ? obj[digit] + 1 : 1;
//       return obj;
//     }, {});
    
//     const cows = guess.split('').filter((digit, index) => {
//       if (secretDigitsCount[digit] && secretDigitsCount[digit] > 0 && guess[index] !== secret[index]) {
//         secretDigitsCount[digit]--;
//         return true;
//       }
//       return false;
//     }).length;
...
function playGame() {
  ...
  while (true) {
    ...
    // if (guess.length !== 4 || isNaN(Number(guess)) || hasDuplicates(guess)) {
    //     // Handle invalid guess
    //   } else {
    //     // Proceed with valid guess
    //   }
