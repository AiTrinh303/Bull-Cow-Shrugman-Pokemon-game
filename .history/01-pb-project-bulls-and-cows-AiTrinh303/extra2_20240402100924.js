...
// 6. Function checks for valid input numbers from the user (no repeat numbers, only numbers, the length of the number should be the same as the level)
function validGuess(guess, level) {
  let valid = true;        
  if (guess.length !== level || isNaN(Number(guess)) || new Set(guess).size !== level) {    console.log('Invalid guess. Please enter again.');
            valid = false;
        }       
         return valid;
      }
