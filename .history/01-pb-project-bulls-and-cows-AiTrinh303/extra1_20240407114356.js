...
function playTheGame(levelArray) {
  let secretNumber = getRandomNumber(levelArra);
  ...
  while (attempts < levelArray[2]) {
    ...
    console.log(
      cows === 1 ? `You found ${cows} cow and` : `You found ${cows} cows and`,
      bulls === 1 ? `${bulls} bull` : `${bulls} bulls`
    );
  ...
  if (attempts === maxAttempts) {
    console.log(`\nSorry, you didn't guess the number within ${maxAttempts} attempts. The number was ${secretNumber}.`);
    let playAgainTheGame = "";
    while (playAgainTheGame.toUpperCase() !== "Y" && playAgainTheGame.toUpperCase() !== "N") {
      playAgainTheGame = prompt("Do you want to play again? 'Y/N': ");
      if (playAgainTheGame.toUpperCase() === "Y") {
...
function start() {
  let [selectedLevel, maxAttempts] = levelSelector();
  while (selectedLevel === false) {
    [selectedLevel, maxAttempts] = levelSelector();
  }
  playTheGame(level, maxAttempts);
}

const [selectedLevel, maxAttempts] = levelSelector();
if (selectedLevel) {
