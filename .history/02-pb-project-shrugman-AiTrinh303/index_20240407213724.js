...
function playGame() {
    ...
    let currentWord = game.getSecretWord(game.category); //get a random word from the selected category


    console.clear();
    console.log(chalk.greenBright(`\n${game.renderWord(currentWord)}\n`));
    ...
    while (game.isGameOn()) {
        ...
        game.update(guess, currentWord);
        console.clear();
        console.log(chalk.greenBright(`\n${game.renderWord()}\n`));
        console.log(chalk.bold.magenta(`\n${game.renderShrugMan()}\n`));

        if (!game.isGameOn()) {
            const isWinning = game.isWinning(currentWord);
            game.updateStats(isWinning, currentWord);

            if (isWinning) {
                console.log(chalk.yellow('Congratulations! You guessed the word!'));
            } else {
                console.log(chalk.redBright('You ran out of attempts! The word was: ', currentWord));
            }
