...
function playGame() {
    ...
    const category = promptSync(chalk.white('Choose a category (movies/books/quotes): '));

    let categoryChoice;   
    if (category === 'movies') {
        categoryChoice = object.keys(options)[0];
    }
    else if (category === 'books') {
        categoryChoice = object.keys(options)[1];
    }
    else if (category === 'quotes') {
        categoryChoice = object.keys(options)[2];
    }
    else {
        console.log(chalk.red('Invalid category. Please choose from movies, books, or quotes.'));
        return;
    }
